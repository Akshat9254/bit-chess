...
void generate_all_knight_legal_moves(const Board *board, MoveList *pseudo_legal_moves) {
    ...
    const Color them = us ^ 1;
    const U8 piece_color_offset = us * PIECE_PER_SIDE;
    const Bitboard target_mask = ~board->occupied[us];
    const Bitboard enemy_mask = board->occupied[them];
    ...
    while (knights) {
        ...
        Bitboard moves = knight_attacks[from] & target_mask;
        while (moves) {
            const Square to = bb_find_lssb_index(moves);
            bb_pop_lssb(&moves);

            const U8 flag = bb_test(enemy_mask, to) ? MOVE_CAPTURE : MOVE_QUIET;
...
void generate_all_bishop_legal_moves(const Board *board, MoveList *pseudo_legal_moves) {
    ...
    while (bishops) {
        ...
        Bitboard moves = get_bishop_attacks(from, all_occ_mask) & target_mask;
...
void generate_all_rook_legal_moves(const Board *board, MoveList *pseudo_legal_moves) {
    ...
    while (rooks) {
        ...
        Bitboard moves = get_rook_attacks(from, all_occ_mask) & target_mask;
...
void generate_all_queen_legal_moves(const Board *board, MoveList *pseudo_legal_moves) {
    ...
    while (queens) {
        ...
        Bitboard moves = (get_bishop_attacks(from, all_occ_mask) | get_rook_attacks(from, all_occ_mask)) & target_mask;
...
void generate_all_king_legal_moves(const Board *board, MoveList *pseudo_legal_moves) {
    ...
    Bitboard moves = king_attacks[from] & target_mask;
    while (moves) {
        const Square to = bb_find_lssb_index(moves);
        bb_pop_lssb(&moves);
